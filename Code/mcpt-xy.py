...
def main():
    ...
    if not os.path.exists('../Results/' + name_dir):
    ...
    #the data sets:
    #change this
    all_data_thermo = np.zeros((nt,number_box*length_box, 4))
    ...
    with Parallel(n_jobs=num_cores, max_nbytes = '5M') as parallel:
        for il in range(number_box):
            ...
            for q in range(nt):
                ...
                for was in range(length_box):                 
                    all_data_thermo[q][length_box*il + ws] = data_extract[ws][0:4]
    ...
    for q in range(nt):
        ...
        np.savetxt('../Results/'+ name_dir +'/configatT='+str(int(temp_init*factor_print)).zfill(5)+'.data',config_at_T[pt_TtoE[q]])
        np.savetxt('../Results/'+ name_dir +'/outputatT='+str(int(temp_init*factor_print)).zfill(5)+'.data',all_data_thermo[q])
        np.savetxt('../Results/'+ name_dir +'/stiffnessPreDataatT='+str(int(temp_init*factor_print)).zfill(5)+'.data',all_data_stiff[q])
        np.savetxt('../Results/'+ name_dir +'/VorticityDataatT='+str(int(temp_init*factor_print)).zfill(5)+'.data',all_data_vort[q])
